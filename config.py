...
class Config:
    ...
    # ──────────────────────────────────────────
    # 해제 인증 (웹 기반 / 일회성 랜덤 코드)
    # ──────────────────────────────────────────
    # 학생이 오버레이의 "해제 요청" 버튼을 누르면 6자리 랜덤 코드가 생성되어
    # 화면에 QR(LAN URL) + 코드가 표시된다. 교수자는 본인 폰으로 QR을 스캔해
    # 학생 PC의 웹페이지에 접속한 뒤, 화면에서 본 코드를 입력하면 잠금이 해제된다.
    # 추후 AWS EC2 + S3 기반 중앙 인증 서버로 확장 예정.

    # 웹 인증 서버 바인드 포트 (LAN 전체에 노출되므로 방화벽 허용 필요).
    WEB_AUTH_PORT: int = 8080

    # 한 번 발급된 해제 코드의 유효 시간(초). 만료되면 무효화되고 요청 버튼이 다시 나타난다.
    UNLOCK_CODE_TTL: int = 120

    # 발급할 해제 코드 자릿수. 6자리 = 100만분의 1 충돌 확률, 단시간 노출에 충분.
    UNLOCK_CODE_LENGTH: int = 6

    # 연속 실패 허용 횟수. 이를 초과하면 활성 코드를 즉시 무효화하여 brute-force를 차단한다.
    # 정상적인 교수자 오타는 1~2회 수준이므로 5는 사용성과 보안 사이의 합리적 절충.
    # 무효화되면 학생이 화면에서 "해제 요청"을 다시 눌러 새 코드를 발급받아야 한다.
    ...
    # 저비용 폴링 주기(초). 포커스 창의 HWND/PID/타이틀만 확인하여 창 전환을 감지한다.
    # 창이 바뀌면 즉시 전체 검사를 트리거하므로 이 값이 곧 "창 전환 → 차단 반응 시간"이다.
    # GetForegroundWindow 등은 microsecond 단위라 1초 미만으로 줄여도 CPU 부하는 미미하다.
    FAST_POLL_INTERVAL: float = 1.0

    # 전체 검사(화면 캡처 + EasyOCR) 주기(초).
    # 창이 바뀌지 않은 상태에서 이 간격마다 1회 OCR을 수행하여,
    # 같은 창 안에서 페이지를 스크롤·이동했을 때 콘텐츠 변화를 잡는다.
    # 값을 키울수록 CPU 부하가 줄지만 같은 창 안에서의 콘텐츠 변화 탐지가 늦어진다.
